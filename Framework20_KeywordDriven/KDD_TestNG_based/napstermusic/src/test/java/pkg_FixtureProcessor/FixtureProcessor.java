        ...
        switch (sFixtureAction.toLowerCase().trim()){
            ...
            case "update_excel_sheet":
                try {
                    ...
                    cellStyleGreen.setFillForegroundColor(new XSSFColor(new java.awt.Color(0, 255, 0)));
                    cellStyleGreen.setFillPattern(FillPatternType.SOLID_FOREGROUND);
                    XSSFCellStyle cellStyleRed   = exlWorkBook.createCellStyle();
                    cellStyleRed.setFillForegroundColor(new XSSFColor(new java.awt.Color(255, 0, 0)));
                    ...
                    exlInStream.close();
                    // prepare to write changes to same file
                    exlOutStream = new FileOutputStream(System.getProperty("user.dir") + sKDDexcelpath);
                    exlWorkBook.write(exlOutStream);
                    exlWorkBook.close();
                    exlOutStream.close();
                    // back to normal processing
        ...
        switch (sFixtureAction.toLowerCase().trim()){
            case "launch_browser":
                try {
                    ...
                    SeleinumProcessor.InvokeBrowser();
